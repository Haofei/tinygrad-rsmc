...
- source size inventory:
  ...
  - integrated `tinygrad-rss/src`: 31 RSS files, 23,740 LOC.
...
- `schedule/__init__.rss`: first integrated source-shaped `tinygrad/schedule/__init__.py` slice:
  ...
  and reject conflicting duplicate values. Full `create_linear_with_vars` orchestration, schedule
  caching, rangeify graph generation, complete linear-call resolution, memory planning integration,
  and JIT capture plumbing remain unported.
